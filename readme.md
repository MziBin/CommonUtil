...
序列化：可以参考别人写的，xml，Soap序列化，Binary序列化等序列化，还有json序列化，就是能够将一个对象存储到文件中，对象里面的是其它对象的数据类型也能存储最好。
...
1.接口通用方法：接口都应该有Init和Dispose这两个方法，用于初始化的调用和结束时的销毁，重写析构函数，在析构函数中调用Dispose。

2.每个保存或者读取的方法都应该有路径这个参数，这样才能更加通用和迁移。

3.什么时候使用单例：当这个类是所有地方都要用，且数据不会冲突的情况下使用，比如获取系统的CPU使用率的系统操作类，像一些每次创建对象都要给不同的参数时，就最好不要使用单例设计模式，最后在每次使用的时候创建新的对象。
